...
export default function Comparativo() {
  ...
  return (
    <div className="py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white">Comparativo do <span className="text-cyan-400">Time</span></h1>
        <p className="text-slate-500 text-sm mt-1">Médias das últimas 20 partidas Flex</p>
      </div>

      <div className="bg-[#0d1117] border border-white/5 rounded-lg mb-6">
        <div className="px-4 py-3 border-b border-white/5 text-xs uppercase tracking-widest text-slate-500">Selecionar métrica</div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={sorted} layout="vertical" margin={{ left: 80 }}>
              ...
              <Bar dataKey={metrica} radius={[0, 4, 4, 0]}>
                {sorted.map((_, i) => (
                  <Cell key={i} fill={i === 0 ? '#0bc4e3' : i === sorted.length - 1 ? '#e84057' : '#334155'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
